...
// Icons maps icon names to their inner SVG path markup.
// Ported from src/common/icons.js.
...
// RankIcon returns the SVG for a rank icon.
// kind is one of "github", "percentile", or anything else for the default
// level icon. percentile is formatted with one decimal place.
// Ported from src/common/icons.js.
func RankIcon(kind, level string, percentile float64) string {
	switch kind {
	case "github":
		return `
	...
	case "percentile":
        ...
        <text x="-5" y="12" alignment-baseline="central" dominant-baseline="central" text-anchor="middle" data-testid="percentile-rank-value" class="rank-percentile-text">
          ` + strconv.FormatFloat(percentile, 'f', 1, 64) + `%
        </text>
      `
	default:
		return `
